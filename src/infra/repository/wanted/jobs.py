...
class IcebergWantedJobsRepository(WantedJobsRepository):
    ...
    def save(self, df: pl.DataFrame, ds: str) -> None:
        ...
        for attempt in range(10):
            try:
                ...
                logger.info(f"[IcebergWantedJobsRepository] saved {len(df)} rows (ds={ds})")
                return
            except CommitFailedException as e:
                if attempt == 9:
                    raise
                wait = min(2 ** attempt + random.random(), 60.0)
                logger.warning(f"[IcebergWantedJobsRepository] commit conflict, retry {attempt + 1}/10 after {wait:.1f}s: {e}")
                time.sleep(wait)
